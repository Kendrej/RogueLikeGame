...
void Assets::transitionImageLayout(VkImage image, VkFormat /*format*/, VkImageLayout oldLayout,
...
IconId Assets::addIconFromFile(const std::string& path)
{
    ...
    transitionImageLayout(s.image, ici.format, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
{
    if (s.imTex)
        ImGui_ImplVulkan_RemoveTexture((VkDescriptorSet)(uintptr_t)s.imTex);
    if (s.sampler)
        vkDestroySampler(ctx.device, s.sampler, nullptr);
    if (s.view)
        vkDestroyImageView(ctx.device, s.view, nullptr);
    if (s.image)
        vkDestroyImage(ctx.device, s.image, nullptr);
    if (s.memory)
...
{
    // Check cache first
    std::string cacheKey = path + "_spritesheet_" + std::to_string(frameCount) + "x" + std::to_string(frameWidth) +
    ...
    {
        stbi_image_free(pixels);
        throw std::runtime_error("Sprite sheet width (" + std::to_string(texW) + ") is less than expected (" +
                                 std::to_string(frameWidth * frameCount) + ")");
    }
    if (texH < frameHeight)
    {
        stbi_image_free(pixels);
        throw std::runtime_error("Sprite sheet height (" + std::to_string(texH) + ") is less than frame height (" +
