...
VulkanImGuiApp::SwapChainSupportDetails VulkanImGuiApp::querySwapChainSupport(VkPhysicalDevice device,
                                                                              VkSurfaceKHR     surface)
{
    ...
    if (fmtCount)
    {
        ...
        vkGetPhysicalDeviceSurfaceFormatsKHR(device, surface, &fmtCount, d.formats.data());
    }
    vkGetPhysicalDeviceSurfacePresentModesKHR(device, surface, &pmCount, nullptr);
    if (pmCount)
    {
        d.presentModes.resize(pmCount);
        vkGetPhysicalDeviceSurfacePresentModesKHR(device, surface, &pmCount, d.presentModes.data());
    }
    return d;
}
...
void VulkanImGuiApp::cleanupSwapchain()
{
    ...
    framebuffers_.clear();
    for (auto& iw : swapchainImages_)
        vkDestroyImageView(device_, iw.view, nullptr);
    swapchainImages_.clear();
    if (swapchain_)
    {
